...
/**
 * Servlet implementation class AjoutReservataireServlet
 */
@WebServlet(name = "AjoutReservataireServlet", urlPatterns = { "/auth/ajoutreservataire" })
public class AjoutReservataireServlet extends HttpServlet {
     ...
     */
    public AjoutReservataireServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			campus = CampusManager.getall();
		} catch (BLLException e) {
			erreurs.addErreur("SystemError", e.getMessages().get(0));
		}
		request.setAttribute("lieux", campus);
		
		request.getRequestDispatcher(URLNavigation.URL_JSP_AJOUT_RESERVATAIRE).forward(request, response);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		verifInfos(snom, sprenom, semail, stelephone, srue, scodePostal, sville, slieu);
		
		reservataire = ReservataireManager.recuperationInstance();
		
		if (erreurs.isOk()) {
			try {
				ReservataireManager.inserer(reservataire);
			} catch (BLLException e) {
		...
		if (erreurs.isOk()) {
			erreurs.setResultat("Réservataire bien enregistré");
	...
	private void verifInfos(String snom, String sprenom, String semail, String stelephone, String srue, String scodePostal, String sville, String slieu) {
		try {
