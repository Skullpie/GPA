...
/**
 * Servlet implementation class ModifCampusServlet
 */

@WebServlet(name = "ModifCampusServlet", urlPatterns = "/auth/modifcampus")
public class ModifCampusServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		Campus campus = null;
		try {
			campus = CampusManager.get(Integer.parseInt(request.getParameter("lieu")));
		...
		if (erreurs.isOk()) {
			request.setAttribute("lieu", campus);
		}
		
		
		request.getRequestDispatcher(URLNavigation.URL_JSP_MODIF_CAMPUS).forward(request, response);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		verifInfos(intitule, rue, codePostal, ville, informationComplementaire, latitude, longitude, code, id);
		
		c=CampusManager.recuperationInstance();
		if (erreurs.isOk()) {
			try {
				CampusManager.majCampus(c);
			} catch (BLLException e) {
		...
		if (erreurs.isOk()) {
			erreurs.setResultat("Campus bien enregistré");
	...
	private void verifInfos(String intitule, String rue, String codePostal, String ville, String informationComplementaire, String latitude, String longitude, String code, String id) {
		try {
